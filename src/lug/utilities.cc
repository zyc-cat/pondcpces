...

{
  ...
  r = new_Effect(e->index);
  r->next = copy_effects( e->next );

  copy_contents_of_FactInfo( &(r->ant->p_conds), e->ant->p_conds );
  copy_contents_of_FactInfo( &(r->ant->n_conds), e->ant->n_conds );
  copy_contents_of_FactInfo( &(r->cons->p_effects), e->cons->p_effects );
  copy_contents_of_FactInfo( &(r->cons->n_effects), e->cons->n_effects );
